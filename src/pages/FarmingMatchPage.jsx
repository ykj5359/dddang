    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <button
          ...
          className="border border-primary-600 text-primary-600 px-6 py-3 rounded-xl font-medium hover:bg-primary-50 transition-colors"
        >
          추가 등록
        </button>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1.5">지목</label>
          <select
            value={form.type}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1.5">면적 (㎡) *</label>
          <input
            type="number"
            required
            value={form.area}
      ...
      <div>
        <label className="text-sm font-medium text-gray-700 block mb-1.5">상세 설명</label>
        <textarea
          value={form.description}
          onChange={(e) => setForm({ ...form, description: e.target.value })}
          placeholder="도로 접면, 수리 상황, 주변 환경 등"
          rows={3}
  ...
  if (saved) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <button onClick={() => setSaved(false)} className="border border-primary-600 text-primary-600 px-6 py-3 rounded-xl font-medium hover:bg-primary-50">
          수정하기
        </button>
      </div>
    );
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          {['매매', '임대', '모두'].map((d) => (
            <button
              key={d}
              type="button"
              onClick={() => setForm({ ...form, dealType: d })}
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="예: 도로 접면 필요, 유기농 가능한 땅, 귀촌 주거 병행 등"
          rows={3}
